...
BigNumber::BigNumber(int mLen, int mode) : len(1), maxLen(mLen) {
    ...
    if (mode == 1) {
        for (int i = 0; i < maxLen; i++) coef[i] = (BASE)(rand() % 16);  
        len = maxLen;
        normalize();
    }
}
...
BigNumber BigNumber::operator+(const BigNumber& bn) const {
    ...
    DBASE k = 0;
    for (int j = 0; j < rL; j++) {
        DBASE a = (j < len) ? coef[j] : 0;
        DBASE b = (j < bn.len) ? bn.coef[j] : 0;
        DBASE t = a + b + k;  
        res.coef[j] = (BASE)(t & 0xFu);  
        k = t >> 4;                        
    }
...
BigNumber BigNumber::operator/(const BigNumber& bn) const {
    ...
    if (bn.len == 1) {
        ...
        for (int j = len - 1; j >= 0; j--) {
            DBASE t = (r << 4) + res.coef[j];         
            res.coef[j] = (BASE)(t / bn.coef[0]); r = t % bn.coef[0];
        }
        res.normalize(); return res;
    }
    DBASE d = 16u / ((DBASE)bn.coef[bn.len - 1] + 1u); 
        ...
        if ((j + n) < u.len) {
            if ((DBASE)u.coef[j + n] < k) {
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    u.coef[i + j] = (BASE)(t & 0xFu);   
                    ak = t >> 4;                         
                }
                u.coef[j + n] = (BASE)ak;
            } else u.coef[j + n] = (BASE)((DBASE)u.coef[j + n] - k);
        }
        q.coef[j] = (BASE)(qh & 0xFu);                  
    }
...
BASE BigNumber::operator%(BASE v) const {
    DBASE r = 0;
    for (int j = len - 1; j >= 0; j--) r = ((r << 4) + coef[j]) % v;   // было << 8
    return (BASE)r;
}
...
BigNumber BigNumber::mul_scalar(DBASE v) const {
    ...
    BigNumber res(len + sizeof(DBASE) * 2, 0);   
    DBASE carry = 0;
    for (int i = 0; i < len; i++) {
        DBASE t = (DBASE)coef[i] * v + carry;
        res.coef[i] = (BASE)(t & 0xFu);            
        carry = t >> 4;                             
    }
    int idx = len;
    while (carry) {
        res.coef[idx++] = (BASE)(carry & 0xFu);    
        carry >>= 4;                                
    }
    res.len = idx;
    res.normalize();
...
ostream& operator<<(ostream& out, const BigNumber& bn) {
    out << hex;  
    for (int i = bn.len - 1; i >= 0; i--) out << (int)bn.coef[i];  
    return out << dec;
}
...
int main() {
    ...
    cout << "SumV: "<< sumV.toDecimal()<<endl;

    BigNumber diff = n1 - n2;
    BigNumber diffV = n1-v;
    cout << "Diff: " << diff.toDecimal() << endl;
    cout << "DiffV: "<< diffV.toDecimal()<<endl;
