    ...
    // ─── Division (big / big) ──────────────────────────────
void BigNumber::divBn(const BigNumber& bn, BigNumber& quotient, BigNumber& remainder) const {
    if (bn.len == 1 && bn.coef[0] == 0) {
        quotient = BigNumber(1, 0);
        remainder = BigNumber(1, 0);
        return;
    }
    ...
    if (bn.len == 1) {
        quotient = *this / bn.coef[0];
        remainder = BigNumber(1, 0);
        remainder.coef[0] = *this % bn.coef[0];
        remainder.len = 1;
        remainder.normalize();
        return;
    }
    ...
    remainder = BigNumber(n, 0);
    for (int i = 0; i < n; i++) {
        remainder.coef[i] = u.coef[i];
    }
    remainder.len = n;
